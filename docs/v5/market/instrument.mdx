...
|[status](../enum#status) |false |string |Symbol status filter <ul><li>`linear` & `inverse` & `spot `By default returns only `Trading` symbols</li><li>`option` By default returns `PreLaunch`, `Trading`, and `Delivering`</li><li>Spot has `Trading` only</li><li>`linear` & `inverse`: when status=PreLaunch, it returns <a href="https://www.abfinance.com/help-center/article/Introduction-to-Pre-Market-Perpetual">Pre-Market contracts</a></li></ul>|
...
<TabItem value="linear" label="Linear/Inverse" default>
    <table border="0.8">
        ...
        <tr>
            <td>> riskParameters</td><td>object</td><td>Risk parameters for limit order price. Note that the <a href="https://announcements.abfinance.com/en/article/adjustments-to-bybit-s-derivative-trading-limit-order-mechanism-blt469228de1902fff6/">formula changed</a> in Jan 2025</td>
        </tr>
        <tr>
