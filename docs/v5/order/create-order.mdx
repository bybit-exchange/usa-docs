...
|isLeverage |false |integer |Whether to borrow. <li>`0`(default): false, spot trading</li><li>`1`: true, margin trading, _make sure you turn on margin trading, and set the relevant currency as collateral_</li>|
...
|reduceOnly |false |boolean |<a href="https://www.abfinance.com/en/help-center/article/Reduce-Only-Order">What is a reduce-only order?</a> `true` means your position can only reduce in size if this order is triggered. <ul><li>You **must** specify it as `true` when you are about to close/reduce the position</li><li>When reduceOnly is true, take profit/stop loss cannot be set</li></ul>Valid for `linear`, `inverse` & `option` |
|closeOnTrigger |false |boolean |<a href="https://www.abfinance.com/en/help-center/article/Close-On-Trigger-Order">What is a close on trigger order?</a> For a closing order. It can only reduce your position, not increase it. If the account has insufficient available balance when the closing order is triggered, then other active orders of similar contracts will be cancelled or reduced. It can be used to ensure your stop loss reduces your position regardless of current available margin.<br/>Valid for `linear` & `inverse` |
