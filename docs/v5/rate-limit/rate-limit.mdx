...
## IP Limit
### HTTP IP limit
You are allowed to send **600 requests within a 5-second window per IP** by default. This limit applies to all traffic directed to `api.abfinance.com`, `api.bybick.com`, and local site hostnames such as `api.bybit.kz`.
...
### Websocket IP limit
* Do not establish more than 500 connections within a 5-minute window. This limit applies to all connections directed to `stream.abfinance.com` as well as local site hostnames such as `stream.bybit.kz`.
