...
## IP限頻
### HTTP IP限頻
默認情況下, 每個IP允許在每5秒的時間窗口內發送最多600次請求。這個速率限制將統計所有打到`api.abfinance.com`, `api.bybick.com`, 以及本地站`api.bybit.kz`等域名的請求數量。
...
### Websocket IP 限頻
* 不要在5分鐘內構建超過500個連接, 這個限頻適用於所有發往`stream.abfinance.com` 以及本地站域名, 比如 `stream.bybit.kz`的連接請求;
* 不要嘗試頻繁地構建連接與斷開連接;
...
<table border="0.8">
    <tr>
        <th rowSpan="2">請求方式</th><th rowSpan="2">路徑</th><th colSpan="3" align="center">統一帳戶</th><th rowSpan="2">是否可提頻</th>
    </tr>
    <tr>
        <th>inverse</th><th>linear</th><th>option</th><th>spot</th>
    </tr>
