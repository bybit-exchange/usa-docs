...
|> X-BAPI-RECV-WINDOW|false |string |默認5000(毫秒). 請求的時間需要滿足該公式: <i>服務器時間 - X-BAPI-RECV-WINDOW <= X-BAPI-TIMESTAMP < 服務器時間 + 1000</i>|
...
- **現貨**的批量接口頻率規則: 請從[這裡](../../rate-limit#不同vip等級的接口限頻規則)查閱其API限頻說明
- ack僅表示請求被成功接受. 請使用websocket-order推送來確認訂單狀態
